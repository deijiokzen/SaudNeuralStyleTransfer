...
from utilities.PyramidForImage import syn_lap_pyr as syn_pyr
from utilities.PyramidForImage import dec_lap_pyr as dec_pyr
from utilities.zca import zca_tensor
...
def filter(x):
    """
    applies modified bilateral filter to AB channels of x, gtranspose_eig_vec_of_whitening_covded by L channel
    ...
    # apply bilateral filtering to AB channels, gtranspose_eig_vec_of_whitening_covdeded by L channel 
    cur = xab.clone()
    for it in range(iterations_maxs):
        cur_pad = get_pad(cur)
        xl_v = {}
        for i in range(3):
            for j in range(3):
...
def match_colors(c, s, o, moment_only=False):
    '''
    ...
    x_new = o.clone()
    for i in range(3):
        x_new[:, i:i + 1,:,:] = range_for_clamp(x_new[:, i:i + 1,:,:], y[:, i:i + 1, :, :])

    _, convolution_for_style = zca_tensor(x_new, y)

    if moment_only or convolution_for_style[1:,1:].abs().max() < 6e-5:
       x_new[:,1:,:,:] = o[:,1:,:,:]
