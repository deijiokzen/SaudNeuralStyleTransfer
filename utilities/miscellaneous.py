...
def get_gpu_memory_map():
    ...
    result = subprocess.check_output(
        [
            ...
            '--format=view_csv,nounits,noheader'
        ], encoding='utf-8')
...
def flatten_grid(xcoord):
    ...
    """
    assert xcoord.size(0) == 1, "undefined behavior for batched input"
    y = xcoord.contiguous().view(xcoord.size(1), -1).clone().transpose(1, 0)
    return y

def scl_spatial(xcoord, h, w):
    """shorter alias for default way I call F.interpolate (i.e. as bilinear
    interpolation
    """
    return F.interpolate(xcoord, (h, w), mode='bilinear', align_corners=True)

def load_path_for_pytorch(path_to_image, target_size=1000, side_comp=max, verbose=False):
    """
    ...
    """
    # Load Image
    x = imread(path_to_image).astype(np.float32)
    ...
    # Removes closefactor channel if present
    if x.shape[2] > 3:
        x = x[:, :, :3]
    ...
    fac = float(target_size) / side_comp(dimensions_for_x[:2])
    h = int(dimensions_for_x[0] * fac)
    w = int(dimensions_for_x[1] * fac)
    x = scl_spatial(x.unsqueeze(0), h, w)[0]

    if verbose:
        print(f'DEBUG: image from path {path_to_image} loaded with size {dimensions_for_x}')
